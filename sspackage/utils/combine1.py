...
class DasBinner:
    ...
    def series_gen(self):
        for a_gen in self.y_gens:
            yt=[(i[0],i[1]) for j in a_gen for i in j if j!=[]]
            yield (yt)
    ...
    #     if squared:
    #         if avg:
    #             for yyy,ttt in self.series_gen():
    #                 m=[sum(i) for i in yyy]
    #                 m2=[sum([j**2 for j in i]) for i in yyy]
    #                 p = [len(i) for i in yyy]
    #                 yield (np.array(ttt), np.array(m), np.array(m2), np.array(p))
    #         else:
    #             for yyyy,tttt in self.series_gen():
    #                 mm=[sum(i) for i in yyyy]
    #                 mm2=[sum([j**2 for j in i]) for i in yyyy]
    #                 yield (np.array(tttt), np.array(m), np.array(m2), np.array(p))
    #     else:
    #             for yyyyy,ttttt in self.series_gen():
    #                 m=(sum(i) for i in yyyyy)
    #                 yield (np.array(ttttt), np.array(m))        
    # # the number of coponents at time t
    # def length_gens(self):
    #     for y,t in self.series_gen():
    #         P=(len(i) for i in y)
    #         yield (P,t)
    # def mass(self, ALL_DAA=False, all=False):
    #     if all:
    #         for T,M in self.mass_gens(True):
    #             yield (T,M)
    #     for TT,MM in self.mass_gens(ResTart=ALL_DAA):
    #         yield (TT,MM)
    # def mass_dev(self, ALL_DAA=False):
    #     # if all:
    #     #     for t,m,m2,p in self.mass_gens(True, True, True):
    #     #         mvar=[(i[1]/i[2])-(i[0]/i[2])**2 for i in zip(m,m2,p)]
    #     #         mdev=[np.sqrt(i) for i in mvar]
    #     #         yield (t,mvar,mdev)
    #     for TTT,MMM,MM2,PP in self.mass_gens(ResTart=ALL_DAA, squared=True, avg=True):
    #         mvar=[(i[1]/i[2])-(i[0]/i[2])**2 for i in zip(MMM,MM2,PP)]
    #         mdev=[np.sqrt(i) for i in mvar]
    #         yield (TTT,mvar,mdev)
